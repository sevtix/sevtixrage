/*mp.events.addCommand("sv", (player, fullText, arg0) => {
    if(player.vehicle) {
      ...
      var fs = require('fs');
      var file = fs.createWriteStream("players/"+player.name+"_"+arg0+".json");
...
/*mp.events.addCommand("lv", (player, fullText, arg0) => {
    ...
    fs.exists("players/"+player.name+"_"+arg0+".json", function(exists){
      let rawdata = fs.readFileSync("players/"+player.name+"_"+arg0+".json");
      c = JSON.parse(rawdata);
      player.notify("Fahrzeug gefunden");
    });

    if(c == null) {
      player.notify("Fahrzeug konnte nicht geladen werden");
      return;
    }
...
/*mp.events.addCommand('dv', (player, fullText, arg0) => {
  if(arg0 != null) {
    ...
    player.setVariable("'" +arg0+ "'", null);
    player.notify("Fahrzeugslot " +arg0+ " wurde gelöscht")
  } else {
